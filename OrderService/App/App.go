...
import (
	...
	"encoding/json"
	"fmt"
	clientPG "gRPCbigapp/ClientService/Adapter/PostgresAdapter"
	"gRPCbigapp/ClientService/Adapter/grpcAdapter"
	clientUC "gRPCbigapp/ClientService/UseCase"
	orderPG "gRPCbigapp/OrderService/Adapters/Postgres"
	orderGRPC "gRPCbigapp/OrderService/Adapters/grpcAdapter"
	"gRPCbigapp/OrderService/Orchestrator"
	"gRPCbigapp/OrderService/Streaming"
	orderUC "gRPCbigapp/OrderService/UseCase"
	"gRPCbigapp/Proto/protoPB"
	authAdapter "gRPCbigapp/Shared/AuthShared/AuthAdapter"
...
func New(ctx context.Context, cfg *Config.Config, logger LoggerPorts.Logger) (*App, error) {
	...
	clientHandler := grpcAdapter.NewUserhandler(clientUseCase, logger, jwtService, grpcAdapter.NewPlanChangePreRequestStub())
...
func (app *App) publishStatusUpdate(_ context.Context, message Kafka.Message) error {
	...
	return nil // todo переделать возврат ошибки под кафку
}
